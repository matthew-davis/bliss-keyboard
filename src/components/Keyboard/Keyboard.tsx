...
import { getMenu } from "../../utils/keyboard";
...
const Keyboard = () => {
  ...
  const getKey = (event: KeyboardEvent) => {
    if (typeof event.keyCode === "number") {
      const location: string = event.location === KeyboardEvent.DOM_KEY_LOCATION_RIGHT ? "-R" : "";
  ...
  const buildKeyboard = (menuKey: number, language: ELanguage = ELanguage.ENGLISH): React.ReactNode => {
    ...
    ["row1", "row2", "row3", "row4", "row5"].forEach((row) => {
      ...
      const buildKey = (key: any, index: any) => {
        ...
        if (["8", "9", "13", "32"].indexOf(key.dataCode) > -1 || !finalClass) finalClass = key.className;
        if (["17-R", "192", "220", "225", "X1", "X2", "X3"].indexOf(key.dataCode) > -1) finalClass = "key--placeholder";
        if (["17", "18"].indexOf(key.dataCode) > -1) finalClass = "key--w1 key--placeholder";
        if (["20"].indexOf(key.dataCode) > -1) finalClass = "key--w3 key--placeholder";
        if (["16", "16-R"].indexOf(key.dataCode) > -1) finalClass = "key--w4 key--placeholder";
        if (["Y1", "Y2"].indexOf(key.dataCode) > -1) finalClass += " key--placeholder";

        // if (!finalClass.includes("placeholder")) menuCharacter = menu[index - 1];
        if (["8", "9", "13", "32"].indexOf(key.dataCode) > -1) menuCharacter = " ";
      ...
      keyboard.push(<div key={row} className="keyboard__row">{keyboardCharacters[row].map(buildKey)}</div>);
    });

    return keyboard;
  };

  return (<div key={"keyboard"} id="keyboard">{buildKeyboard(menuKey, language)}</div>);
};
